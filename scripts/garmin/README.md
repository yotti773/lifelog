...
> **移行メモ(Issue #215・#216、2026-08-23):** アプリの同期先が「サービスアカウントで共有した既存シート」から
> 「アプリがユーザーのDriveに作成するシート」へ変わった。**このスクリプトの書き込み先は自動では変わらない。**
> アプリと同じシートに活動記録を書き続けるには、(1) アプリが作った新しいシートをサービスアカウントのメール
> アドレスに編集者として共有し、(2) GitHub Actions の Secret `GOOGLE_SHEETS_SPREADSHEET_ID` を新しいシートの
> IDへ差し替える。**Actions の Secrets 自体は削除しないこと**(Worker のシークレットと同名だが別物)。
