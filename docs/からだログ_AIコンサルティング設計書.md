...
interface WeeklyDigest {
  ...
  bloodPressure?: {                             // 血圧の週サマリー(フェーズ4、Issue #117)。記録が無い週は省略
    avgSystolic: number;                         // 週平均の最高血圧(記録がある日の平均)
    avgDiastolic: number;                        // 週平均の最低血圧
    recordedDays: number;                        // 血圧記録がある日数(0〜7)
    highReadingDays: number;                     // 家庭血圧135/85以上だった日数(事実提示。医学的判断はしない)
    weekAvgWeightKg: number | null;              // 同じ週の週平均体重(体重×血圧を並べて見せる。#112の軸)
  };
...
| `INTAKE_OVER_TARGET` | 両方 | 週平均摂取カロリーが目標を10%以上超過し、かつ目標以内に収まった日が2日以下(週次のみ。#210。**食事記録が5日未満の週・目標カロリー未設定の週は判定しない**) |
| `FAT_OVER_TARGET` | 両方 | 週平均脂質が目標を30%以上超過している(週次のみ。#210。脂質目標未設定の週は判定しない) |
| `PROTEIN_UNDER_TARGET` | 両方 | 週平均たんぱく質が目標を20%以上下回っている(週次のみ。#210。**不足側のみ**判定し、超過側は判定しない。たんぱく質目標未設定の週は判定しない) |
...
- `pfc.mostOffTargetNutrient` は目標から最も外れている栄養素(Issue #210)。目標・実績とも値がある栄養素のうち `(実績-目標)/目標` の絶対値が最大のものを1つ選ぶ(`aggregatePfcDeviation()`、`src/lib/weeklyDigest.ts`)。`INTAKE_OVER_TARGET`・`FAT_OVER_TARGET` とは独立に、目標が設定されていれば常に計算する(超過が無い週でも「たんぱく質が最も不足している」のような事実提示に使える)。**摂取が目標を超えた週(`INTAKE_OVER_TARGET`)の背景を具体的に示すための項目**で、実例(2026-08-09、8/3週)は総超過258kcalが脂質+32g(288kcal)でほぼ説明がつく週だった。判定条件:
  ...
  - **`INTAKE_OVER_TARGET` には食事記録5日以上のデータ品質ゲートを課す**(#174の歩数と同じ考え方)。`daysOnTarget` は**食事記録がある日**を分母にした値のため、ゲートが無いと記録2日・両日とも超過の週で `daysOnTarget=0` となり「週を通して超え続けた」と誤判定する。しかもこのフラグはverdict規則の先頭(`needs_attention`)にあり `INSUFFICIENT_DATA`(`slightly_behind`)を上書きするため、**「データが少ない週は評価に適さない」という既存の防御を突き破ってしまう**(実装中のレビューで発見)。閾値は `LOW_RECORDING_THRESHOLD_DAYS` と同じ5日
  - `FAT_OVER_TARGET` は脂質だけを独立して判定する。総カロリーの超過は目標比の相対値では栄養素間の重みが分かりにくいため、「脂質を減らす」という具体的な行動に直結する項目を単独でフラグ化した
  - `PROTEIN_UNDER_TARGET` は**不足側のみ**判定する(多い分には実害が乏しいため)。減量中のたんぱく質は筋量維持に直結し、取るべき行動も明確なためフラグ化した。閾値の−20%は、目標が体重1kgあたり2.0g前後で設定される前提だと減量中の推奨下限(1.6g/kg)にあたる
  - **炭水化物・脂質不足はフラグ化しない。** 炭水化物の超過は `INTAKE_OVER_TARGET` がほぼ捕捉し、PFCの中で炭水化物は「P/Fを決めた残り」の性格が強く単独では行動が定まらない。不足側は意図的な低炭水化物運用がありえて「不足=問題」と断定できない(血圧の高値・activityの水準をフラグ化しなかったのと同じ「解釈の領域」の判断)。事実提示は `mostOffTargetNutrient` が担う
  - `FAT_OVER_TARGET`・`PROTEIN_UNDER_TARGET` には記録日数ゲートを課さない(平均値そのものの比較で、`daysOnTarget` のような日数カウントに依存しないため)
  - 閾値(10%・2日以下・記録5日以上・脂質+30%・たんぱく質−20%)は運用しながら調整する前提の初期値(#174の閾値群と同じ扱い)
  - **`INTAKE_OVER_TARGET` と `PROTEIN_UNDER_TARGET` が同時にある週の扱いを5章のプロンプトで明示する。** 実例では、たんぱく質不足の事実だけを見たAIが「摂取量を高めて」と提案し、1日258kcal超過している週への助言として字面が矛盾していた。この組み合わせでは量を増やす提案をさせず、総摂取カロリーを増やさない「置き換え」の提案に固定する(週次レビュー画面にも同趣旨の注記を出す。`WeeklyReview.tsx` の `proteinSwapNote`)。#174の `INTAKE_BELOW_BMR` × `ACTIVITY_DROP` と同じ、フラグの組み合わせによる注記
  - **「脂質を減らして」と名指しできるのは `FAT_OVER_TARGET` も立っている週だけ**(実装中のレビューで発見)。炭水化物主導の超過で脂質が目標未満の週にこの文言を出すと、**目標を下回っている栄養素を削れと言うことになり**、すぐ上のPFCバー(脂質が目標未満)と矛盾して見える。画面は文言を出し分け、プロンプトは「減らす対象として名指ししてよいのは実際に目標を超過している栄養素だけ」と条件付きで指示する
  - MonthlyDigestは `pfc` を持たないため、月次レビューへの適用(`FAT_OVER_TARGET`・`PROTEIN_UNDER_TARGET` の判定材料が無い)は本Issueでは見送り、別途判断する(`MonthlyDigestFlag` から明示的に除外)
...
- **システムプロンプト(固定)**:
  ...
  - `verdict` の判定基準: 必要ペースとの乖離・フラグの有無から機械的に選べるよう、条件を列挙して指示する。**判定規則は `DigestFlag` の全値を網羅すること** — 規則のどこにも現れないフラグが単独で立った週は、「flagsが空 → `on_track`」にも当てはまらず `verdict` が決まらなくなる(フラグ追加時に見落としやすいため、`weeklyAdvice.test.ts` が全値の出現を検証している)
