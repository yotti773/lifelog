...
// verdictの表示は週次(WeeklyAdviceCard.tsx)と同じ4値→ラベル・3色
...
/**
 * 月次AIコーチのコメントカード(Issue #114)。生成はユーザーの明示操作でのみ行い、
 * 生成済みの月はキャッシュ(monthlyAdviceRecords)を表示する。出力契約は週次と共通で、
 * winsを「今月の良かった変化」、actionsを「来月の重点」として表示する。
 */
      ...
      {isGenerating ? (
        ...
        <>
          <Typography sx={{ fontSize: 12, color: "text.secondary", lineHeight: 1.7, mb: "12px" }}>
            この月の実績サマリーをもとに、今月の変化と来月の重点をAIが提案します
          </Typography>
          <Button
            fullWidth
            variant="contained"
