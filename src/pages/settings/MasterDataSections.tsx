...
import { IconBarbell, IconCheck, IconFork } from "@/components/icons";
import { db } from "@/db/db";
import { tokens } from "@/theme";
import SettingRow, { SectionLabel } from "./SettingRow";

/** 設定画面の「食事マスタ」「種目マスタ」「習慣マスタ」セクション。件数表示と管理画面への遷移を持つ */
export default function MasterDataSections() {
  ...
  const habitMasterCount = useLiveQuery(() => db.habitMasterItems.count(), []);
  ...
  return (
    <>
      ...
      <SectionLabel>習慣マスタ</SectionLabel>
      <Card sx={{ overflow: "hidden", mb: "18px" }}>
        <SettingRow
          ...
          iconColor="#2EC4B6"
          label="記録する習慣を管理"
          value={habitMasterCount !== undefined ? `${habitMasterCount}件` : ""}
          onClick={() => navigate("/settings/habit-master")}
        />
      </Card>
    </>
  );
