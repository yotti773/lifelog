...
## Overview
食事・体重・体脂肪率などのライフログを記録するモバイルアプリのMVP。ホーム（当日サマリー）、記録フロー（体重/食事）、推移（グラフ/履歴）、設定（目標管理）、食事マスタ管理の6画面 + アクションシート/編集ボトムシートで構成。React + MUIでの実装を想定して本資料を作成。
...
## 画面構成
すべてモバイル1画面（375〜392px幅想定）。共通シェル：ステータスバー（実装不要、OSが描画）+ ボトムナビゲーション（ホーム/推移/設定の3タブ、設定タブは食事マスタ画面でもアクティブ表示）+ ホーム画面のみ右下にFAB。
...
- 食事リスト: 朝食/昼食/夕食/間食の4行。記録済みはカード(shadow)＋行右端にシェブロンがあり、タップで食事編集画面へ遷移。未記録は破線ボーダー＋タップでFABと同じアクションシートを開く
...
| FAB | `Fab` (color="primary") | 右下 fixed、onClickでSpeedDial的挙動 or 自前Drawer |
...
**状態遷移**:
- `openActionSheet()` ← FABタップ or 未記録の食事行タップ
- `openMealEdit(mealId)` ← 記録済みの食事行タップ → 「7. 食事を編集」画面へ遷移
...
### 2. 推移画面（Trends）
**Purpose**: 体重・体脂肪率・摂取カロリーの推移と、目標までの進捗を確認する。グラフ/履歴のサブタブを持つ。

**Layout**:
- ヘッダー固定（sticky）: タイトル + セグメントコントロール（グラフ/履歴）
- **グラフタブ**: ゴールバーカード → 体重推移グラフ → 体脂肪率推移グラフ → カロリー推移グラフ（縦積み、gap 14px）
- **履歴タブ**: From/To絞込みチップ2つ + リセットボタン → 件数テキスト → リスト（Card内、行ごとborder-bottom）→ 0件時は空状態メッセージ
...
| From/Toチップ | `Chip` or `ButtonBase`で自作カード | タップでDatePicker系ボトムシートを開く |
| リセットボタン | `IconButton` (`RestartAltIcon`) | |
| 履歴リスト行 | `List` + `ListItem` | 基準日バッジは`Chip size="small"` |
| 空状態 | 自作`Box`（中央寄せテキスト） | |
...
| `records` | WeightRecord[] | 履歴タブの元データ（日付・体重・体脂肪率・メモ・基準日フラグ） |

**状態遷移**:
- `tab: 'graph' | 'history'`（ローカルstate）
- `historyFrom`, `historyTo`（Date range、ローカルstateまたはURLクエリ）→ `records`をこの範囲でfilterした`filteredRecords`をuseMemoで算出
- From/Toチップタップ → 日付選択ボトムシート（`editTarget: 'from' | 'to'`）を開く → 確定で対応するstateを更新
- リセットボタン → `historyFrom`/`historyTo`をデフォルト（全期間 or 直近30日など要件次第）に戻す

---

### 3. 設定画面（Settings）
**Purpose**: 目標値・データ同期・食事マスタ・（旧JSONバックアップは削除済み）を管理する。

**Layout**:
- セクション見出し(12px, #8C8C8C) + `Card`でグルーピングされた行リスト、の繰り返し
- セクション: 「目標」（4行）→「データ同期」→「食事マスタ」（2行）
...
| プロパティ | 型 |
|---|---|
| `goalWeight`, `goalDate`, `baselineDate \| null`, `goalCalories` | 編集対象の4値 |
| `lastSyncAt`, `unsyncedCount` | データ同期セクション |
| `mealMasterCount` | 「よく食べるものを管理」の件数バッジ |

**状態遷移**:
- `editMode: null | 'weight' | 'calories' | 'goalDate' | 'baseline'` → 該当行タップで対応するボトムシートを開く
- 各ボトムシート内は `draft*` の一時state（ステッパー±ボタン + 直接入力`TextField`の両方で編集可能）→「保存」で親stateにコミット、「キャンセル」で破棄
- 基準日のみ「未設定にする」を持つ（`baselineDate: null`許容。未設定時は一番古い体重記録を起点に自動フォールバックする仕様 — 画面設計書6章参照）
- 目標カロリーの保存はホーム画面のカロリー計算・進捗バーに反映、目標日の保存は推移画面の「あと◯日」表示に反映（グローバル状態 or Context経由）
...
**Props**: `mealType`（初期値は現在時刻から算出）, `aiSuggestion`（写真解析結果、任意）, `masterItems`
**状態遷移**: 区分切替、写真アップロード→AI呼び出し(非同期, loading state)→フォーム自動入力、マスタ行タップ→フォーム上書き、保存→ホームへ遷移

---

## グローバル状態・ナビゲーション
- `screen: 'home' | 'trends' | 'settings' | 'master' | 'weight' | 'meal'` — ルーティング（`react-router`のpathに置き換え推奨。プロトタイプはSPA内state切替のみ）
- `sheetOpen: boolean` — ホームのアクションシート（体重/食事の選択）
- `editMode` 系 — 設定画面の4つの目標編集ボトムシート、履歴のFrom/To編集ボトムシート
...
- `03_trends_history.png` — 推移（履歴タブ・From/To絞込み）
- `04_settings.png` — 設定
- `05_meal_master.png` — 食事マスタ管理
- `06_weight_entry.png` — 体重を記録
- `07_meal_entry.png` — 食事を記録
- `08_meal_edit.png` / `09_meal_delete_confirm.png` — 食事を編集・削除確認（Issue #24、`prototype.dc.html`のみに実装。未同梱のため必要なら追加キャプチャしてください）
