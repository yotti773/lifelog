...
/**
 * 食事判定用のプロンプトを組み立てる。写真判定とテキスト一行入力(Issue #159)の両方をこの1関数でまかなう。
 * photoCountが0なら写真無し(テキストのみ)とみなし、写真前提の文言をテキスト前提に置き換える。
 *
 * 軽量モデル(Flash-Lite級)でも読み取り精度が出るよう、以下の構成にしている(Issue #62):
 * - 品目の特定 → 分量(g)の見積もり → 栄養値の推定、という思考順序を番号付き手順で固定する
 * - 日本の標準的な1人前の分量アンカーを提示し、写真・テキストの内容に応じてスケールさせる
...
export function buildMealJudgmentPrompt(mealLabel: string, note?: string, photoCount = 1): string {
  ...
  // 写真ありなら備考(note)は「補足情報」、写真無し(テキストのみ)ならnote自体が判定対象の本文になる
  const textSection = hasPhoto
    ? trimmedNote === ""
      ? ""
      : `
## ユーザーからの補足情報(最優先)
...
- 複数の写真に同じ料理が写っている場合(別アングル・撮り直しなど)は、重複させず1品として1回だけ数える。写真ごとに別の料理が写っている場合はすべて挙げる`
  ...
  const intro = hasPhoto
    ? `あなたは日本の食事に詳しい管理栄養士です。${photoContext}。写っている料理・食品を判定し、品目ごとの分量とカロリー・PFC(たんぱく質・脂質・炭水化物)を推定してJSONで返してください。`
    : `あなたは日本の食事に詳しい管理栄養士です。次のテキストは${mealLabel}の内容です。書かれている料理・食品を判定し、品目ごとの分量とカロリー・PFC(たんぱく質・脂質・炭水化物)を推定してJSONで返してください。`;

  const step1 = hasPhoto
    ? "1. 写真に写っている料理・食品をすべて挙げる。見分けられる料理が複数あれば(例: 唐揚げ・ご飯・味噌汁)、まとめずに1品ずつitemsの別の要素に分ける。単一の料理なら1件だけにする"
    : "1. テキストに書かれている料理・食品をすべて挙げる。見分けられる料理が複数あれば(例: 唐揚げ・ご飯・味噌汁)、まとめずに1品ずつitemsの別の要素に分ける。単一の料理なら1件だけにする";

  const step2 = hasPhoto
    ? "2. 品目ごとに重さ(g)を見積もり、estimatedWeightGに入れる。食器・箸・手など写っているものとの大きさの比較で判断し、判断できなければ日本の家庭料理・外食の標準的な1人前とする"
    : "2. 品目ごとに重さ(g)を見積もり、estimatedWeightGに入れる。テキストに個数・量の記述があればそれを手がかりにし(例:「ゆで卵2個」なら卵1個分×2)、記述が無ければ日本の家庭料理・外食の標準的な1人前とする";

  const amountAnchorHint = hasPhoto ? "写真の量に応じて増減させる" : "テキストの記述に応じて増減させる";

  const uncertainRule = hasPhoto
    ? "- isUncertain: 写真が不鮮明、量や内容の判定に自信が低い、または複数の料理をやむを得ず1つの品目にまとめた場合はtrue。それ以外はfalse"
    : "- isUncertain: テキストの記述が曖昧で量や内容の判定に自信が低い、または複数の料理をやむを得ず1つの品目にまとめた場合はtrue。それ以外はfalse";
...
## 分量の目安(標準的な1人前。${amountAnchorHint})
...
- 揚げ物・炒め物・外食メニューは、衣・調理油・ドレッシングなど見落としがちな油分も脂質とkcalに含める(過小評価しやすいので注意)
