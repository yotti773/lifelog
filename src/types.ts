...
export interface Settings {
  ...
  /**
   * AI機能(週次・月次のAIコメント、食事の写真・テキスト判定)でGoogle Gemini APIへ
   * データを送ることへの同意日時(ISO8601。Issue #219)。**未設定=未同意**。
   *
   * boolean ではなく日時にしているのは、同意の記録として「いつ同意したか」が要るため。
   * 撤回するとこの項目自体を消す(false を入れない — 「一度も同意していない」と
   * 「同意して撤回した」を区別する意味が無く、未設定=未同意の判定を1つに保てる)。
   *
   * **スプレッドシート同期の対象にしない**(`apiToken` と同じ扱い)。設定の取り込みは
   * 「ローカルで未設定の項目だけを埋める」ため、同期対象にすると**撤回した同意が次の取り込みで
   * 復活してしまう**(設定には削除の仕組みが無く、シート側の行を消せない)。
   * 端末を変えたら同意を取り直す、というのが同意の記録としても正しい。
   */
  aiConsentAgreedAt?: string;
}
